...
struct Set2Test7View: View {
    @Environment(\.dismiss) private var dismiss
    @EnvironmentObject private var router: NavigationRouter
    @EnvironmentObject private var resultsStore: ResultsStore
    @Environment(\.modelContext) private var modelContext
    @StateObject private var viewModel = Set2DistanceTaskViewModel()
                        ...
                        GeometryReader { geometry in
                            ZStack(alignment: .leading) {
                                ...
                                Capsule()
                                    ...
                                    .frame(width: geometry.size.width * CGFloat(viewModel.timeRemaining / Set2DistanceTaskViewModel.totalDuration))
...
private extension Set2Test7View {
    ...
    func goToNextTest() {
        recordResultIfNeeded()
        router.go(to: .set2Test1)
    }
}

// MARK: - View model
final class Set2DistanceTaskViewModel: ObservableObject {
    @Published private(set) var questions: [Set2DistanceQuestion]
    ...
    init(questions: [Set2DistanceQuestion] = Set2DistanceQuestion.defaultQuestions) {
        self.questions = questions
    }

    var currentQuestion: Set2DistanceQuestion? {
...
struct Set2DistanceQuestion: Identifiable {
    let id = UUID()
    let prompt: String
    let options: [String]
    let correctIndex: Int

    static let defaultQuestions: [Set2DistanceQuestion] = [
        Set2DistanceQuestion(
            prompt: "The distance from aircraft 32 to point Kilo is:",
            options: ["4 NM", "2 NM", "7 NM", "10 NM"],
            correctIndex: 0
        ),
        Set2DistanceQuestion(
            prompt: "The distance from aircraft 19 to point Xray is:",
            options: ["1,5 NM", "2 NM", "2,5 NM", "3 NM"],
            correctIndex: 2
        ),
        Set2DistanceQuestion(
            prompt: "The distance from aircraft 19 to point Golf is:",
            options: ["10 NM", "8 NM", "6 NM", "12 NM"],
            correctIndex: 0
        ),
        Set2DistanceQuestion(
            prompt: "The quickest way for aircraft 71 to get to point Echo is:",
            options: ["Turn left to heading 045", "Turn right to heading 090", "Turn left to heading 090", "None"],
            correctIndex: 0
        ),
        Set2DistanceQuestion(
            prompt: "The quickest way for aircraft 66 to get to point Xray is:",
            options: ["Turn right Heading 090", "Turn left heading 270", "Turn right to heading 270", "Turn left heading 090"],
            correctIndex: 2
        ),
        Set2DistanceQuestion(
            prompt: "The quickest way for aircraft 32 to get to point Echo is:",
            options: ["Turn right to heading 045", "Turn left to heading 045", "Both A and B are right", "Turn left heading 090"],
            correctIndex: 2
        ),
        Set2DistanceQuestion(
            prompt: "The quickest way for aircraft 19 to get to point Golf is:",
            options: ["Turn right to heading 135", "Turn left to heading 135", "Turn right heading 090", "Turn left heading 090"],
            correctIndex: 1
        ),
        Set2DistanceQuestion(
        ...
        Set2DistanceQuestion(
            ...
            options: ["6 NM", "10 NM", "8 NM", "12 NM"],
            correctIndex: 0
        ),
        Set2DistanceQuestion(
            prompt: "The distance from aircraft 71 to point Kilo is:",
            options: ["8 NM", "6 NM", "10 NM", "12 NM"],
            correctIndex: 0
        )
    ]
}
