...
struct DemoTest7View: View {
    @Environment(\.dismiss) private var dismiss
    @EnvironmentObject private var router: NavigationRouter
    @EnvironmentObject private var resultsStore: ResultsStore
    @Environment(\.modelContext) private var modelContext
    @StateObject private var viewModel = DemoDistanceTaskViewModel()
                        ...
                        GeometryReader { geometry in
                            ZStack(alignment: .leading) {
                                ...
                                Capsule()
                                    ...
                                    .frame(width: geometry.size.width * CGFloat(viewModel.timeRemaining / DemoDistanceTaskViewModel.totalDuration))
...
private extension DemoTest7View {
...
// MARK: - View model
final class DemoDistanceTaskViewModel: ObservableObject {
    @Published private(set) var questions: [DemoDistanceQuestion]
    ...
    init(questions: [DemoDistanceQuestion] = DemoDistanceQuestion.defaultQuestions) {
        self.questions = questions
    }

    var currentQuestion: DemoDistanceQuestion? {
...
struct DemoDistanceQuestion: Identifiable {
    let id = UUID()
    let prompt: String
    let options: [String]
    let correctIndex: Int

    static let defaultQuestions: [DemoDistanceQuestion] = [
        DemoDistanceQuestion(
