...
struct DemoTest2View: View {
    @Environment(\.dismiss) private var dismiss
    @EnvironmentObject private var router: NavigationRouter
    @EnvironmentObject private var resultsStore: ResultsStore
    @Environment(\.modelContext) private var modelContext
    @StateObject private var viewModel = DemoCompassTaskViewModel()
        ...
        VStack(alignment: .center) {
            ...
            if testStarted {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    HStack(spacing: 12) {
                        Spacer()
                        ForEach(CompassDirectionDemo.allCases, id: \.self) { direction in
                            Button(action: {
                    ...
                    GeometryReader { geometry in
                        ZStack(alignment: .leading) {
                            ...
                            Capsule()
                                ...
                                .frame(width: geometry.size.width * CGFloat(viewModel.timeRemaining / DemoCompassTaskViewModel.totalDuration))
    ...
    private func compassColumn(title: String, pointer: CompassDirectionDemo?, isReference: Bool, isNavod: Bool, centerLabel: String?) -> some View {
        VStack(spacing: 12) {
            Text(title)
                .font(.headline)
            if isReference {
                DemoCompassGraphic(centerLabel: centerLabel, isReference: isReference, isNavod: isNavod, pointerDir: pointer)
...
final class DemoCompassTaskViewModel: ObservableObject {
    @Published var currentDirection: CompassDirectionDemo? = nil
    @Published var currentLetter: CompassDirectionDemo? = nil
    ...
    static let totalDuration: TimeInterval = 20
    ...
    private func presentNewTrial() {
        guard !showResults else { return }
        nextTrialWorkItem?.cancel()
        currentDirection = CompassDirectionDemo.allCases.randomElement()
        currentLetter = CompassDirectionDemo.allCases.randomElement()
        trialStartTime = Date()
    }
...
enum CompassDirectionDemo: String, CaseIterable, Equatable {
    case s = "S" // sever
    case z = "Z" // západ
    case j = "J" // jih
    case v = "V" // východ
...
private extension DemoTest2View {
    ...
    func proceedToNextTest() {
        recordResultIfNeeded()
        router.go(to: .demoTest3)
    }
}

struct DemoCompassGraphic: View {
    var centerLabel: String?
    var isReference: Bool?
    var isNavod: Bool?
    var pointerDir: CompassDirectionDemo?
    var displayedLetter: String? = nil
    let directions = ["S", "Z", "V", "J"]
    
    private let letterOffsets: [CompassDirectionDemo: CGPoint] = [
        .s: CGPoint(x: 0, y: -1.3),
        .z: CGPoint(x: -1.3, y: 0),
        .j: CGPoint(x: 0, y: 1.3),
        .v: CGPoint(x: 1.3, y: 0)
    ]
    private let letterOffsetsNavod: [CompassDirectionDemo: CGPoint] = [
        .s: CGPoint(x: -1.2, y: 0)
    ]
    ...
    private func pointerPath(center: CGPoint, radius: CGFloat, direction: CompassDirectionDemo) -> Path {
        var path = Path()
