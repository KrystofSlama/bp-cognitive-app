...
struct Set3Test7View: View {
    @Environment(\.dismiss) private var dismiss
    @EnvironmentObject private var router: NavigationRouter
    @EnvironmentObject private var resultsStore: ResultsStore
    @Environment(\.modelContext) private var modelContext
    @StateObject private var viewModel = Set3DistanceTaskViewModel()
                        ...
                        GeometryReader { geometry in
                            ZStack(alignment: .leading) {
                                ...
                                Capsule()
                                    ...
                                    .frame(width: geometry.size.width * CGFloat(viewModel.timeRemaining / Set3DistanceTaskViewModel.totalDuration))
...
private extension Set3Test7View {
    ...
    func goToNextTest() {
        recordResultIfNeeded()
        router.go(to: .set3Test1)
    }
}

// MARK: - View model
final class Set3DistanceTaskViewModel: ObservableObject {
    @Published private(set) var questions: [Set3DistanceQuestion]
    ...
    init(questions: [Set3DistanceQuestion] = Set3DistanceQuestion.defaultQuestions) {
        self.questions = questions
    }

    var currentQuestion: Set3DistanceQuestion? {
...
struct Set3DistanceQuestion: Identifiable {
    let id = UUID()
    let prompt: String
    let options: [String]
    let correctIndex: Int

    static let defaultQuestions: [Set3DistanceQuestion] = [
        Set3DistanceQuestion(
            prompt: "The distance from aircraft 32 to point Golf is:",
            options: ["6 NM", "8 NM", "4 NM", "10 NM"],
            correctIndex: 0
        ),
        Set3DistanceQuestion(
            prompt: "The distance from aircraft 71 to aircraft 66 is:",
            options: ["18 NM", "12 NM", "24 NM", "20 NM"],
            correctIndex: 2
        ),
        Set3DistanceQuestion(
            prompt: "The distance from aircraft 19 to point Golf is:",
            options: ["16 NM", "12 NM", "12 NM", "14 NM"],
            correctIndex: 0
        ),
        Set3DistanceQuestion(
            prompt: "The distance for aircraft 66 to point Xray is:",
            options: ["18 NM", "22 NM", "16 NM", "20 NM"],
            correctIndex: 0
        ),
        Set3DistanceQuestion(
            prompt: "The distance from aircraft 66 to aircraft 44 is:",
            options: ["12 NM", "8 NM", "10 NM", "6 NM"],
            correctIndex: 0
        ),
        Set3DistanceQuestion(
            prompt: "The quickest way for aircraft 71 to get to point Echo is:",
            options: ["Turn left Heading 045", "Turn right to heading 090", "Turn left to heading 360", "None"],
            correctIndex: 0
        ),
        Set3DistanceQuestion(
            prompt: "The quickest way for aircraft 32 to get to point Golf is:",
            options: ["Turn right to heading 090", "Turn left to heading 315", "Turn left to heading 090", "Turn left to heading 360"],
            correctIndex: 2
        ),
        Set3DistanceQuestion(
            prompt: "The quickest way for aircraft 44 to get to point Kilo is:",
            options: ["Turn right Heading 090", "Turn right heading 300", "Turn left heading 300", "Turn right heading 360"],
            correctIndex: 2
        ),
        Set3DistanceQuestion(
            prompt: "The quickest way for  aircraft 19 to get to point Golf is:",
            options: ["Turn right Heading 135", "Turn left heading 135", "Turn right heading 090", "Turn left heading 090"],
            correctIndex: 1
        ),
        Set3DistanceQuestion(
            prompt: "The quickest way for aircraft 44 to point Xray is:",
            options: ["Turn right Heading 270", "Turn left heading 300", "Turn right heading 300", "Turn left heading 270"],
            correctIndex: 1
        )
    ]
}
